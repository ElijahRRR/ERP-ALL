...
## 增量验证流程（2026-07-18 起：先分支后合并；2026-08-01 起按 D-Q74 分级）

**L2 增量**（渠道写路径 / 花钱 / 删数据 / 安全边界）一律先在 PR 分支上验证，通过后 Owner
授权合并 main，再继续开发；**L0/L1 不单独验**，随下一次例行部署**批量点验**——部署后跑一遍
冒烟清单（首页 / 产品 / 订单 / 定时任务面板各开一次 + healthz），有异常再回溯到单
（见 `specs/009-process-lite/README.md` §四）。main 恒为「CI 绿；L2 增量另经真机验过」：
