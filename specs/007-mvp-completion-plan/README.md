...
- **07c 邮箱域最小闭环**（✅ **阻塞已解除**：Owner 2026-07-27 确认已有可接的真实邮箱
  与 IMAP 凭证；模块 7 最后一块，MVP 剩余路径第 2 站）：§09 mail 域图纸（IMAP/SMTP
  客户端模式，非自建服务器，D-Q22）——收件入库+店铺关联+Walmart 通知 LLM 分类（`llm_usage.module=mail_classify`
