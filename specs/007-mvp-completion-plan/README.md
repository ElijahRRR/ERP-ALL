...
## 单人模式（D-Q73，2026-07-30——**读任何工单前先读本节，它修改所有工单的默认前提**）

Owner 裁定：**「我只需要一个我个人能够使用的 erp」**——没有团队页、没有角色页、
知道地址就能操作；流水账保留（我操作的/定时任务的/后台报错的）。

**执行方式＝表层摘除、地基休眠**（实测 19 张表 team_id / 102 条 RLS / 106 个权限端点，
拆除是数周手术，休眠运行时零成本——超管身份在前后端各一行短路放行一切）：

### R2-17 单人模式收口【新工单，P0，先于三路并行的一切】

- **17a 免登录**：`SINGLE_USER_MODE` 配置开关（默认关，部署机开）。开启时
  `get_current_user` 对无凭证请求直接返回固定超管身份（`core/authn.py`，超管 GUC 注入
  沿现路径）；前端检测该模式跳过 LoginPage。**流水账不受影响**：UI 操作记固定用户，
  beat 记 `actor_type='system'`，报错走 incidents/notifications——三流本就分开。
- **17b 导航摘除**：UsersPage / RolesPage / TeamSwitcher 从导航与路由摘掉（文件保留）。
- **17c 插件共享 token**：`plugin_instance` 签发/吊销流程不做；一个共享常量 token
  （.env 一行，构建进插件）。表保留可空置。
- **17d 指派简化**：订单页人工下拉指派采购账号；`customerId` 首见自动建档即 `active`
  （**取消 `pending_claim` 闸**——单人操作没有"待认领"仪式的对象）。

**验收**：①部署机开 `SINGLE_USER_MODE` 后，浏览器直开地址即达产品页，无登录；
②audit_log 三流可辨（人工操作/定时任务/系统报错各取一条验 actor 归属）；
③导航无 团队/用户/角色 入口；④插件带共享 token 可拉任务，不带则拒（唯一保留的闸）；
⑤关掉开关，登录流程原样回来（休眠可逆的证明）。

**对存量工单的修改**：
- **R2-13**：验收③（越权三条）**整条划除**；13a 的实例认证段按 17c 执行；13b 的
  自动路由策略降级为人工指派（`daily_cap` 列保留但不强制消费）。
- **R2-14**：已合并的 14a/14b 不动（能力在，无害）；**14d 其余主体面不再做**。
- **R2-16 / R2-07c / R2-08**：判据中涉及权限点/越权的字句划除，其余不变。
- **RS-01/RS-12**：维持挂账，启动条件=开第二团队或对外。
- **新工单纪律**：一律不再写 越权/权限点/角色 判据。

⚠️ **唯一风险注记（说一次，不再重复）**：免登录=拿到地址即可操作店铺。
**仅限内网/VPN，绝不把 Web 端口映射到公网**。RS-02a 已把 db/redis 绑 127.0.0.1，
Web 端口自此是唯一边界。

## 三路并行开工须知（Owner 2026-07-30：R2-13 / R2-14 14b / R2-16 并行）

> ⛔ **本节整节被冻结令挂起（见「MVP 剩余路径」节首）**：R2-17+R2-18 完成并部署前，
> 三路一律不开新工。本节内容（迁移号预分配、依赖、冲突面、合并次序）在解冻后照用，
> 但迁移号顺延：R2-17 若需迁移取 `0050`，三路原号段不变。
...
## MVP 剩余路径（✅ 定案，Owner 2026-07-27：D-Q66/67/68；**2026-07-30 冻结令覆盖其上**）

> ⛔ **冻结令（Owner 2026-07-30）：「我需要先执行项目简化再进行下一步的开发」。**
> **R2-17（单人模式收口）+ R2-18（流程分级落地）完成并部署之前，下表所有功能线一律
> 不开新工、在途的停在当前提交**。理由：不先简化，每张新单又会带出团队/角色/权限的
> 纠缠；且 17c（共享 token）直接取代 13a 的实例签发设计——**先定简化，插件线少走返工**。
> 唯一例外：线上事故修复（L2 判级里的保险丝不冻结）。
> 解冻标志：R2-17 验收①③④过 + 部署机回执 + 本节删除此段。
...
| 序 | 事项 | 性质 | 备注 |
|---|---|---|---|
| **-1** | **R2-17 单人模式收口 + R2-18 流程分级**（D-Q73/74） | ⛔ **冻结其余一切** | 17a 免登录 / 17b 摘导航 / 17c 插件共享 token / 17d 人工指派；R2-18 可并行（纯 docs）。**完成并部署后其余各行才解冻** |
| **0** | **RS-02a 端口与口令**（P0 即刻） | 开发 AI 改 compose | **不排队、插队做**：db/redis 绑 127.0.0.1、换 PG 默认口令、Redis `requirepass`、默认密钥硬失败。依据 D-Q68 实测。**单人模式后 Web 端口是唯一边界，本单更重要而非更轻** |
| 1 | R2-09 三档自动化贯通 | 在推 | 前置已解除（flow v2 冻结） |
| 2 | ~~R2-07c 邮箱最小闭环~~ | ⏸ **搁置（Owner 2026-08-01「这一步放着」）** | 不进解冻后排期。D-Q71 选型结论（imapclient/IMAP ID/代理随店铺/保守节流）与 A/B 实测证据**全部在档**，重启时直接用，无需重新调研 |
...
**挂账/推后**（不进 MVP）：**R2-07c 邮箱底座（Owner 2026-08-01 搁置，选型与实测证据在档待重启）**、
R2-05 发货环节（Owner 2026-07-27 决定暂不动，等真实新单）、
...
**合并前闸序（2026-07-26 起；2026-08-01 起按 D-Q74 分级适用）**：CI 绿 →
**审查 AI 通读 diff** → 部署机真机验证 → **Owner 授权合并**。
分级口径见 `specs/009-process-lite/README.md` §一：**L2（渠道写/花钱/删数据/安全边界）
走上述全套**；L1 免单独真机（随部署批量点验）；L0 免审查。
**「Owner 授权合并」一级不减免**——任何级别的 PR 都由 Owner 合并。
审查 AI 位于 Owner 拍板之**前**，其价值即保护该次授权决策。
