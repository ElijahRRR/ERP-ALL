...
| purchase_execute | 采购执行 | manual/semi/auto | **创建快照** | **采购任务下发给采购插件**：人工点采 / 半自动（待确认后下发）/ 全自动下发（R2-13，2026-07-27 补入 v2.1）。**auto 档花的是真金白银**，必须带护栏 | amount_ceiling（单单金额上限）、price_delta_pct（较预估涨价超阈值即转人工）、delivery_days_limit（预计送达上限天数）——**均为业务策略**。⚠️ **本 config 不含 `daily_cap`**（2026-07-30 移除）：日采购上限是**账号物理属性**不是业务策略，唯一落点为 `buyer_account.daily_cap`；团队级的「今天要不要采」由三档档位承担（人工档即不自动派发），不需再设数字上限 |
