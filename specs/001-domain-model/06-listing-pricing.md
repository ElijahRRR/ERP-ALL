...
| payload | JSONB | NOT NULL | **单个 MPItem 元素模板**（build={Visible,Orderable}；match={Item}）——feed 级 header 是提交时组装，不入产品级缓存（R2-03 修订：原「最终提交体」含 header 的设计随构建器真实化调整）。SKU/GTIN/价格/库存/日期/PartnerID 为 listing/store 级参数经占位符实例化注入。已落地（R2-11 增量2）变体段两层切分：成员维度值（映射后）进模板与 build_hash 指纹（改维度自动失效缓存，未分组指纹不变）；variantGroupId=VG{组id} 与 variantAttributeNames 为组级参数走实例化注入不进缓存键；isPrimaryVariant 一律不传且全链剥离（D-Q63）。提交侧 _submit_tx1 组级三闸（D-Q64③ 修订：broken/anchor/单批上限 `variant.max_batch_members`——「组不齐」家族完整性检查废止，子集即可成组/追加上架）批次原子拒绝原因可见，本批一员失败整批撤除；提交可选 `variant_mode ∈ {group, standalone}`（D-Q64②，散品模式不带 VG 段不锁 anchor）；live 散品补挂走 `item_regroup` feed（D-Q64④，独立归位：失败不动 listing 状态/不释放 GTIN）；match 模式全程豁免变体段（D-Q3 参数包差异） |
