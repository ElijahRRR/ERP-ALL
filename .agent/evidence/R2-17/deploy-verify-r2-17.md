# R2-17 单人模式收口 ③闸真机验证单 v1（验收①—⑤；D 编号，与清场单 C 编号不混）——已收口 ✅

> **收口记录（2026-08-01）**：D1—D7 全部 PASS（D8 回执已贴 PR #50）。一次停点：
> D3 首轮 401——compose `backend_env` 锚点未转发 17a/17c 新增的三个 `ERP_` 变量
> （部署面缺口，非产品失败），`6c447e6` 补转发后续跑全过。验收真机取证：
> ①免登录 `/me` 无凭证 200 + admin/is_super + 浏览器直达工作台；③导航四项均无；
> ④token_len=44 带/不带 401↔200 对照 + `A17VERIFY` 首见即 active（17d 顺带取证）；
> ②三流可辨（system|auto_register / user / notifications=50）；⑤关开关 401 +
> 登录页回归 + 复开恢复（休眠可逆）。终态：验证分支 `6c447e6`（四项 CI 绿，
> run 30693847609）、DB `0048`、单人模式常驻开启、API 健康。
> **解冻标志（R2-17 gate 原文「验收①③④过+部署机回执」）四件真机取证齐备**；
> 解冻判定与合并授权仍分属审计侧与 Owner——闸序余项＝②审查 + ④Owner 授权合并。
