...
> **插件已 vendored 进本仓 `amz-plugin/`**（不再是独立 fork 仓）——E1 检出 ERP-ALL
> 分支时它就一起下来了，无需再 `git clone`/`checkout` 另一个仓。以下路径均相对
> `D:\项目文件\ERP-ALL\amz-plugin\`。

**手工两步**（本单唯一允许的本地改动；不 commit 不 push）：
1. **凭证走 gitignore 的本地配置，绝不改 tracked 文件**（③闸真机实测：编辑 tracked
   `popup.js` 注入 token 后，只读 `git diff` 就把活值打印到终端＝凭证泄露）。
   ```powershell
   cd D:\项目文件\ERP-ALL\amz-plugin
   Copy-Item js/config.local.example.js js/config.local.js
   ```
   编辑 `js/config.local.js`：`token` 填 `.env` 里 `ERP_PLUGIN_SHARED_TOKEN` 的值；
   `baseUrl` 默认 `http://127.0.0.1:8000/api/v1` 一般不改。`config.local.js` 已 gitignore
   （`amz-plugin/.gitignore` 与仓根各钉一道），`git status` 不应出现它——若出现即**停**。
