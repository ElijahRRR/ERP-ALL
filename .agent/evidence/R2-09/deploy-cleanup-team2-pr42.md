...
## ⚠️ 安全性靠调用参数，不是 SQL 本身（审查 2026-07-28 三轮指出；本文 v1/v2/v3 通用）

SQL 正文里**没有** `BEGIN;`/`COMMIT;`——四道闸只是 `DO $$ ... RAISE EXCEPTION $$;`。
「任一闸中止即整体回滚」这句话成立，**完全依赖调用行的两个 flag**：

```
psql -v ON_ERROR_STOP=1 -1 ...
```

`-1` = `--single-transaction`，配 `ON_ERROR_STOP=1`，`RAISE` 才回滚整段。
**若走默认 autocommit 又不带这两个 flag，闸报错只中止该条语句、后面的 DELETE 照常执行**
——那就是「闸全在、却一条都没挡住」。

**所以：绝不能为了「先看看闸过不过」而拆掉 `-1` 单独跑 SQL**，那会把闸降级成告警。
要预演就整段照跑——闸不过时它本来就什么都不删（v1/v2 两次中止即实证）。
...
4. **不许拆 `psql -1 / ON_ERROR_STOP=1`**（见上节；这是本文全部安全性所在）。
