...
> 合并前闸序**第三闸**。第一闸 CI 绿已过（4/4 @ `2c2ab65`）；第二闸独立审查**两轮通过**
> （F1 反向判据缺口与 N2 enabled 必填均已修，F1 修法经审查侧起临时 worktree 变异实测确认）。
>
> **本单与 #41 有三点不同，先记住再动手**：
...
验不出授权）。没有这种账号就先用超管在用户管理页建一个/挂上角色，**贴回所用用户名**（不贴口令）。
开页面前 **Ctrl+F5**。
...
- **C 三态渲染**：`unset` 灰「未配置」/ `disabled` 橙「已停用」/ `configured` 档位 Tag /
