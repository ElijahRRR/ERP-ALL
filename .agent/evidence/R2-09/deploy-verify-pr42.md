...
# 第三闸回执（2026-07-28，**全部通过**）

分支 `44acfba`（内容 = 当前 PR 代码面）· DB 升 `0040` 验完降回 `0039` · 终态 `main@1f7f742`。

| 段 | 结果 |
|---|---|
| 前置①② | `TRACKED_DIRTY_EXIT=0`；两个 `rev-parse` 一致，无 ahead/behind/diverged |
| 第 1 步 | `UP_EXIT=0`，migrate `Exited (0)` |
| 第 2 步 | `WIRED= 3` / `OK` / `PROBE_EXIT=0`；`HEALTH_CODE=200` |
| 第 3 步 | `version_num=0040`；权限码 2 行；角色 14 行、模板与 team 1 团管 `automation_codes` 各 2；`policy_rows=0`；`total_teams=1` |
| 第 4 步 A | 五项全过 |
| 第 4 步 B | B1/B2/B反向/B补充 全过（含「非法 semi 点人工仍弹红确认」） |
| 第 4 步 C | unset / configured / disabled / 非法档位 / 未接线 全过；**C-unset 10 行全 0** |
| 第 4 步 D | D-1 四条全过（菜单在、10 行可见、只读标签、控件全灰）；D-2 两条全过（菜单消失、`/automation` 403） |
| 第 5 步 | 审计 10 行，全 `team_id=1` / `actor_id=6` / `automation.policy_set`；`AUDIT_EXIT=0` |
| 第 6 步 | `DELETE 3`（三行与本轮所造一致）；`pr42_ro` 已停用；临时角色权限清空；**既有团管角色未触碰**；审计一行未删 |
| 第 7 步 | `DOWNGRADE_EXIT=0` → `0039` / `automation_codes=0`；`HEAD == origin/main == 1f7f742`；反向探针 `ModuleNotFoundError`（`MAIN_PROBE_EXIT=1`，**符合预期**）；`MAIN_HEALTH_CODE=200` |

**审计段的自洽性值得记一笔**：`187` 是 `pricing_watch` 且 `is_first_write=f`，而更晚的 `188` 反是 `t`
——因为 `187` 属**上一轮**验证（当时行存在），上一轮清理删行后本轮再 PUT，`before` 又为 NULL
故重新计首次写入。三个 `t`（188/189/196）正对应本轮三条 flow 的首次落库。
**判据在「跨轮次删除后重建」这种非平凡时序下仍自洽**，不是靠单调递增蒙对的。

## 本轮暴露的四个问题（全部不在被验对象上）

| # | 现象 | 定性 | 处置 |
|---|---|---|---|
| 1 | C 段返回 0 行 | **我的指令**：中文字面量经 PowerShell 管道进 psql 匹配不上 | 取证 SQL 去中文谓词 |
| 2 | C-unset 报 8/10 失败 | **浏览器持有修复前 chunk**（镜像本身是修复版） | UI 判据前置加「产物对拍」机器判据 |
| 3 | `crypto` / `atob` 未定义 | **部署机工具层的 JS 运行时缺全局对象**，我的 PowerShell 一行未跑到 | D 改走 UI 首选路径，零脚本 |
| 4 | D 缺非超管凭据 | **不必要的阻塞**：超管足以自造账号 | 写进指令：凭据自助、口令只落本机 |

共同点：**四次都是我给的路径或环境差异，被验对象一次都没被碰到**；四次都被部署侧的
「报错即停、不自行扩大、不改了再试」挡住，没有一次变成误操作或误判。

## 遗留（不阻塞合并）

- 临时口令明文文件 `D:\项目文件\pr42-ro-secret.txt` 仍在。账号已停用、角色已清空，凭据已惰化；
  **建议删除该文件**（不再需要），由 Owner 定。
- `pr42_verify`（team 1，团管+采集员）与 `pr42_ro`（已停用）两个验证账号保留在库中。
