...
- Task（当前手上这张）: **R2-14 生命周期出口 14a+14c【L1】——Owner 2026-07-29 指定开工，
  代码已落、闸序中**。口径 `001§7.1`（硬删除三级规则）+ 007 分片（14a/14c）。
  - 增量：**14c-1** 列表默认折叠 `retired` + 开关（显式 status 筛选优先，不叠 AND）→
    **14a-1** 迁移 0041（`deleted_product` 墓碑 + `catalog.product_delete` 权限点 +
    补授 `listing`/`listing_spec` 的 DELETE）→ **14a-2** 删除服务 +
    **仓内第一个 DELETE 端点** → **14a-3** 入库点接墓碑（验收②承重件）→
    **14a-4** 判据 16 条 + 前端删除入口二次确认 + 台账回写。
  - 覆盖验收 **①②④⑤⑥**；**③（用户删除 + `deleted_principal`）属 14b，不在本轮**。
  - 待 Owner/审计侧裁定两条（不阻塞合并，但影响下一轮）：
    (a) **墓碑不可撤销 → 误删不可恢复**（实体已物理消失、重采被墓碑挡），是否开
    「恢复采集」入口属范围决策；(b) 墓碑比图纸多存 `product_id` / `master_sku`
    （理由同 `deleted_principal`：`audit_run.product_id` 无外键，删完只剩裸 id），请确认或改判。
  - 另记一条**图纸内部张力**（不阻塞，本轮按图纸实现）：①级不写墓碑（§7.1 与验收①都明写），
    但「墓碑保去重」这层作用对①级同样成立——Owner 清理 team 2 那 400 个**从未上架**的产品
    正落在①级，删完再采一轮仍会全部回流。§7.1 把墓碑的两个用途（保历史可读 / 挡回流）
    合在一处论证，落到①级时只保住了前者。
- Task（前一张，已合并）: **R2-09 三档自动化贯通【L1】——2026-07-27 立项**。
...
- 全局挂账：R2-05 L2 发货补验（等 A152 真实来单）；R2-04 验收②模拟断连；钓鱼黑名单导入；
  ...
  **已清偿（2026-07-28）**：部署机生产转储 `erp_all-before-0039.dump` 已删——回执
  `REMOVE_OK=True` + `Test-Path=False` + 删后 `.dump` 清点为空、仅删指定文件
  （Owner 裁定「删」；误提交路径此前已用 .gitignore 堵死）。
...
- **DV-0731 挂账（PR #50 六轮审查提出的检查器盲区，云端侧采纳）**：四检查器
  （psvar/xref/payload/psquote）都不校验「部署指令里出现的 `app.<表>.<列>` 是否真实
  存在于迁移链」——I1（v7 拿从未建过的 `note` 列当清理键，⑫单事务清理会整段回滚）正
  落在该盲区。候选第五检查器：从指令 md 的 psql 载荷抽表/列引用，与迁移到 head 后的
  information_schema 对表。附带小项：xrefcheck 的步号词表只认①-⑫，v8 新增的⓪不在
  扫描集。两项都不阻 R2-13 ③闸，待本单收账后随下一张指令类工单落地。
