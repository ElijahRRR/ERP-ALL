...
## 2026-07-28 真机验证第一轮：6 项 PASS，卡在我编错的健康路径

部署机执行 v3，**前置②③④⑨⑩⑪ 全部通过**。两条最要紧的判据都过了：

- **`FLOWS= 10` / `KERNEL_EXIT=0`**——容器跑的确实是分支代码，不是旧镜像缓存；
- 切回后**负向判据如期失败**（`ModuleNotFoundError`，`ROLLBACK_KERNEL_EXIT=1`）——切换真实生效。
- migrate `Exited (0)` 且**未执行新迁移**，与「零迁移」相符。

「正向 import 成功 / 负向 import 必须失败」这对判据设计是有效的：它同时回答了
「跑的是不是新代码」和「切回有没有真生效」，且完全是退出码判断、与平台无关。

### 卡点：`/api/health` 是我凭记忆编的，那个路径根本不存在

真实端点是 **`/healthz`**（`backend/src/erp/main.py:110`，**无 `/api` 前缀**；业务路由才挂 `/api/v1`）。
部署机报回「分支与 main **都是 404**」——**两边都 404 恰好证明这不是回归而是我路径写错**，
而且它没有自行改试其他路径。它的处理无可挑剔。

**这是同一个毛病的第三次**：`git status` 判据写宽、`checkout` 拿到陈旧分支、健康路径凭记忆编。
前两个是「我以为那台机的状态是某样」，这个是「我以为代码里有这个端点」——
**共同点是没去核，就写进了给别人执行的指令**。SQL 我核了（先在沙箱实跑），端点没核。
下次写任何指令，凡是引用了「代码里的具体名字」（路径、模块、字段、命令），都要 grep 一遍再写。

### 顺带发现第四处：第 6 步 `git checkout main` 与前置那个 bug 同源

回执⑨显示切回后落在 **`1986bb1`** 而不是远端 `1f09edf`——`git fetch origin main` 只更新
`origin/main`，**不动本地 main**。我刚在前置为验证分支修了这个 bug，**却没意识到第 6 步
有一模一样的一处**。而且我第 6 步的判据只看 `git log -1`、**没有比对 `origin/main`**，
所以它没被抓住——**判据没覆盖到的地方，错误就不会自己冒出来**。
已改为 `git checkout -B main origin/main` + 两个 rev-parse 自校验。

### 剩余步骤的一处简化（省一整轮重建）

步骤 3（现网数据新旧判定对拍）与步骤 4（判据自检）是**纯只读 DB 查询，与部署哪个分支无关**
——`app.automation_policy` 里的数据不会因为跑的是分支还是 main 而不同。
**所以这两步可以直接在当前 main 上跑，不必为它们重挂分支。**
步骤 5（扫日志里的 `automation.*` 告警）确实需要分支代码在跑，但**若步骤 3 的 C 段返回 0 行，
它必然无匹配**，可省。

这个简化不是偷懒——是把「哪些判据依赖被测代码、哪些不依赖」分清楚。
一开始把它们排成线性六步，是没想清楚这件事。

## 2026-07-28 真机对拍结果是空集——不能算「验过了」

A 段跑完：**`total_rows=0`**。该机的 `app.automation_policy` 是**空表**（仅 2 个团队，
`order_block` 全为「无行」）。于是 `divergent_gate=0 / divergent_mode=0` 是**空真**。

**必须如实说：这一步没有产出任何关于新旧行为等价的证据，不能计为「对拍通过」。**
拿一个「比较了 0 行，发现 0 处分歧」去充当「行为等价已验证」，就是我一直在防的那种
「判据全绿但什么都没证明」。

### 第 4 步的判据自检恰好在这里发挥了作用

`total_rows=0 = checked_rows=0`、`total_teams=2 = 第四段 2 行`——它把
**「0 因为表空」**与**「0 因为查错了库/表」**区分开了。这正是当初加它的全部理由。
如果没有它，我今天会看着三个 0 说「对拍通过」，而实际上连查没查对表都不知道。
**给判据配自检，收益就在这种时候兑现。**

### 它实际产出的是另一条有用事实

该机现网**没有任何自动化策略行**——所有团队所有 flow 都是 manual，`order_block` 全部
「只软标记不冻结」，四检 flagged 的订单不会被冻结。

**这与 D-Q14「合规命中=软标记，自动拦截开关由同事自行开启」的缺省一致，不是缺陷**，
但意味着拦截能力在现网**一处都没启用**。这个数以前没人查过，运营侧应当知情。

由此可推一条**窄**结论：对这台机而言本次重构**不可能改变行为**（零行时新旧读点都回 manual）
——但这是「因为没数据」，不是「因为验过了」。**行为等价的真实依据仍只有 CI 与审查侧的
33 组差分实测**。审查侧当时用临时 PG 簇造了 11 组库状态，**恰好补上了现网空表的覆盖缺口**
——这也说明「造数据的合成测试」与「拿现网数据对拍」不是替代关系，是互补关系。

### 第五处指令错：健康检查没等就绪

B 段 `/healthz` 返回 `000`、curl `exit 52`。**52 是「空响应」不是「连接被拒」（那是 7）**
——`docker compose up -d --build` 在容器创建完就返回，此时 uvicorn 常还没绑上端口，
而 Docker 的端口转发会先接受 TCP 连接再关闭。**我没写等待就绪。**
已改为轮询 30 次 × 2s，并注明「curl 退出码在轮询里不是判据，最终状态码才是」。

五处指令错的分布也值得看一眼：判据写宽 1、命令选错 2（fetch 不更新本地分支，两处同源）、
凭记忆编路径 1、缺就绪等待 1。**没有一处是业务逻辑错，全是「我以为环境是某样」。**
而部署机五次全部严格照做、及时停手、无一次自作主张。

## 2026-07-28 第三闸收尾：4 项拿到实证，1 项无覆盖

部署机最终回执 `HEALTH_CODE=200 (after 1 tries)`——**同时实证了 `/healthz` 是正确路径**
（v4 修正有效），机器留在 `main@1f09edf`、状态正常。

**逐项分类，不混着说**：

| 项 | 结论 | 依据 |
|---|---|---|
| 容器跑的是分支代码 | ✅ 实证 | `FLOWS= 10` / `KERNEL_EXIT=0`（该模块 main 上不存在） |
| 切换真实生效 | ✅ 实证 | 切回后负向判据如期失败：`ModuleNotFoundError` / exit 1 |
| 零迁移属实 | ✅ 实证 | migrate `Exited (0)`，未执行新迁移 |
| 服务健康、可回正常态 | ✅ 实证 | `HEALTH_CODE=200`，main 对齐 `1f09edf` |
| **现网数据新旧对拍** | ❌ **无覆盖** | `automation_policy` 空表 → 空真，不计通过 |

最后一项**物理上无法在这台机补齐**：补它只能往生产库造策略行再对拍，属「往生产写业务数据」，
为一次验证不值得。等价性由 CI + 审查侧 33 组差分实测承担——**审查侧那 11 组人造库状态，
恰好覆盖了现网空表覆盖不到的区间**。

### 这一闸真正的产出，可能是那条副产物

「现网一条自动化策略行都没有，`order_block` 全部只软标记不冻结」——这个事实以前没人查过。
它与 D-Q14 缺省一致、不是缺陷，但**拦截能力在现网一处都没启用**，运营侧应当知情。
本闸设计时把它当作 D 段的「顺带看看」，结果它比主判据更有信息量。

### 过程代价要记下来

本闸往返 **5 轮**，**5 处指令错全在云端侧**：判据写宽 1、fetch 不更新本地分支 2（同源）、
健康路径凭记忆编 1、缺就绪等待 1。**没有一处是业务逻辑错，全是「我以为那台机的环境是某样」。**
部署机 5 次全部严格照做、及时停手、无一次自作主张——分工是有效的，但**指令质量决定往返次数**。

可复用的一条：**凡是指令里引用「另一侧环境中的具体名字或状态」（路径、模块、分支、就绪时机），
写之前必须核**。这次 SQL 我核了（沙箱实跑），端点和 git 状态没核——错的正是没核的那几处。

顺带自记一笔：写这段回写时，我自己又因为上一条命令的 `cd backend` 残留把三条命令跑在了
错目录（`GATE_EXIT=2` + file-not-found）。**这条我昨天刚写进自检提醒，转头又犯**——
说明「写进提醒」和「执行时真的检查」是两件事，前者不自动带来后者。

## 2026-07-28 R2-09 增量1 关账：PR #41 合入 main `4b668e4`

Owner 裁定「收口合并」。四道闸为本单**首次走满且全部留痕**：

| 闸 | 结果 |
|---|---|
| ① CI | 四项全绿 @ `c36f6f1` |
| ② 独立审查 | 通过——33 组差分实测零分歧、四种图纸漂移变异全红、事务边界核过；F1 已修 |
| ③ 部署机真机 | **4 项实证 + 1 项无覆盖**（详见上条） |
| ④ Owner 授权 | 2026-07-28「收口合并」 |

**那「1 项无覆盖」写进了 PR 正文，也写进了合并提交信息**——没有埋在四个 ✅ 里。
这一点是本单最该保住的东西：**合并授权基于正文，正文就不能只说通过的部分**。

### 回头看这一单，最值得留下的三条

**1. 「manual = 最保守」在闸类 flow 上不成立。** 内核初版把「档位对本 flow 非法」归 manual，
那对 `order_block` 是**放松**——它的 `auto` 才是拦截、`manual` 是只软标记不冻结。
若照初版落地，任何存着 `semi` 的团队会**静默失去订单拦截**，而本单声明的是「行为零变化」。
这个错是在写判据时自己抓出来的，不是审出来的——**写判据比写实现更容易暴露语义错误**。

**2. 判据要能回答「你到底查了什么」。** 真机对拍返回三个 0，如果没有第 4 步的自检
（`total_rows == checked_rows`），我会把「比较了 0 行」读成「对拍通过」。
**空真是最像成功的失败。**

**3. 合成造数与现网对拍是互补不是替代。** 审查侧用临时 PG 簇造的 11 组库状态，
恰好覆盖了现网空表覆盖不到的全部区间；而现网对拍产出了合成测试永远给不出的事实
——现网一条策略行都没有，拦截能力一处都没启用。

### 下一步

增量2：策略读写 API + 权限点 `automation.read/write` + 前端策略面板 + Q3 三条
（三态显性区分 / 停用记 warn【已落】/ 闸类停用二次确认）。**面板不做模板按钮**（Q2 终裁）。
无前置，分支已从 `4b668e4` 重建。

## 2026-07-28 合并后追记：PR #41 实际 12 文件（正文说 10）——第五次正文失实，载体是 add -A

stop-hook 报 dirty 牵出来的。`4b668e4` 实际 squash 了 **12** 个文件：正文清单之外多了
真机指令文件（正当新增、清单没更新）和 `R1-11/dry-run-feed-snapshot.json` 的**测试漂移**。

后者是机制性的：该快照由测试每次运行重写，EAN 随沙箱库池状态在 `…0021`↔`…0038` 间摆动
（CI fresh DB 恒为 `0021`，即 canonical），而我的 `git add -A .agent` 习惯把每次摆动都
扫进了提交——漂移在多个提交间来回翻转，纯噪音进了 main。

「代码只在一个 commit/5 个文件」的不变量仍为真，但**文件数失实成立**。五次正文失实里，
前四次是「正文是快照而 head 在动」，这一次是**「我不知道自己提交了什么」**——更糟的一种。

**流程修正**：①提交显式列文件，弃用 `git add -A .agent`；②pytest 后、commit 前先还原
该快照的漂移。根治（测试确定化）另行提单。

## 2026-07-28 部署机生产转储已删（挂账销）

回执三项判据全对：`REMOVE_OK=True`（cmdlet 用 `$?` 判——上次 `$LASTEXITCODE` 的坑没重犯）、
**`Test-Path=False`**（决定性判据：文件真的没了）、删后 `.dump` 清点为空且仅删指定文件。
至此该转储的两条风险线都闭合：误提交路径（.gitignore，07-28 上午）+ 文件本身（本次删除）。

## 2026-07-28 R2-09 增量2：策略 API + 权限点 + 面板（workflow 编排，19 条发现全处置）

11 个 opus/max 子代理（4 勘察→1 设计→2 实现→3 镜头审查→1 变异证伪）产出实现，
审查与证伪层抓出 **19 条去重发现**——其中 6 条是合并前必修的真货，全部修复后
全量 651 passed / 1 skipped、前端 lint+build 绿、0040 往返 exit 0。细目在
review_list R2-09 updates[13]，这里只记三条把「为什么这么做」讲透的：

### 证伪层的价值又一次兑现：T7 审计判据有出口

变异 M4（删掉审计写入）在冷库上 T7 红——但**红的理由是错的**（断言的是上一轮遗留的
审计行）。温库上连跑两次后再施加变异，T7 **判绿**：查询无本次运行的界定，尾部恰好是
匹配的一对就满足。**CI 每轮起新库，冷库掩护着这个出口**——「判据被 CI 环境救着」和
「判据自身成立」是两回事。修法：max(id) 水位线 + 恰好 2 行。修后温库连续两轮变异皆红。

### 「面板说安全、实际不安全」三连——显示层是新的 fail-open 面

三镜头同向抓出：compliance_block 零消费点却被断言「拦截生效」（ERP_ENV 同形）；
闸类确认漏掉 effective=semi（唯一「显示异常但闸真开着」的状态恰好不设防）；
GET 对无团队超管返回一屏「全 unset+红字」假面板（描述的不是任何团队，是「没有团队」）。
**内核层这次是干净的（审查原话：三层做得扎实），错全在显示层**——增量1 把 fail-open
从执行层赶出去，增量2 差点让它从显示层回来。修法都是「把事实源交给服务端」：
WIRED_FLOWS 代码事实注册表 + gate/wired 字段下发，前端不再自己判断任何安全语义。

### 统一错误信封被自己的防御措施绕过

extra="forbid" 和 min/max_length 都是「防御性」写法，但它们的错误走 FastAPI 默认
{detail} 信封——**为了拒绝 config 而把 002 统一信封绕过了**，且 T10 只看状态码抓不到。
防御措施本身也要问一句「它失败时走哪条路」。修法：显式接住再用 BusinessError 拒绝，
空串/超长/bogus 统一收敛到 AUTOMATION_MODE_ILLEGAL 一个码。
app 级 RequestValidationError 信封缺口（缺 mode 等结构违例，全仓皆然）另行提单不混本单。
