...
### 写回四端点（2/3/4/7）的越权面：**本团队全部插件派发单**（定性留档，非缺陷）

本文件 `_TEAM_PLUGIN_PO_SQL` 的谓词是 `team_id = :t AND buyer_account_id IS NOT NULL`
——**不含任何账号级条件**。故一把令牌（哪怕它这一刻带的 `customerId` 从未解析到任何
账号，例如待认领/已驳回/根本没见过）**可以写回本团队任意一张插件派发单**：回填单号与
金额（端点 2）、把单打成 `exception`（端点 3）、盖 `exception_kind`（端点 4）、写运单与
轨迹（端点 7）。这不是疏漏，是三条事实叠出来的**已知面**，逐条记在这里免得下一个人
「顺手收窄」：

1. **图纸明说它不在威胁模型内**——`07:338-340` 逐字：「团队内一台浏览器"冒充"同团队
   另一个 `customerId` 不在威胁模型内：那是自家机器、自家运营，且它只能用**实际登录
   的那个**亚马逊账号付款，冒充只会让自己拿到买不了的单——是操作事故不是越权。真正的
   重复下单风险由「同一订单只派一个账号」+ `claimed` 原子锁挡住。」**真正的边界是跨团队**
   （`team_id = :t` + RLS 两层都在），那一条有承重用例钉着。
2. **厂商载荷根本不带 `customerId`**（协议如此，逐字取证见
   `.agent/evidence/R2-13/owner-reverse-engineering-20260730.md` C.2/E.2 与 `schemas.py`）
   ⇒ 写回路径**无从**按身份收窄：没有输入可比。硬要收窄只能拿「实例最近登的号」
   （`last_seen_customer_id`，0044 头注六写明**观察列不参与鉴权**）去比，那等于把一个
   会被下一次拉取覆盖的观察值升成鉴权凭据。
3. **收窄的代价是钱**：一台浏览器拉完单后换登了别的号，它手上那笔**已经花出去的钱**
   仍然必须报得回来。按账号收窄会让那次回填 403 ⇒「钱花了系统不知道」——比「同团队内
   浏览器串号」严重得多。

**若审计侧要求账号级粘性（如「令牌一旦解析到账号 A 就只能写 A 的单」），必须先改图纸
的威胁模型**（`07:338-340`）——那是设计变更，走批注回传，不由本文件单方面收窄；且同一
份改动要一并给出「换号后旧单怎么报回来」的通道，否则只是把越权面换成资金黑洞。
...
# 买不了的单）。
#
# ⚠️ **本谓词的越权面写在明处**（审查 H1 定性留档，非缺陷）：**没有账号级条件** ⇒ 一把
# 令牌可写回**本团队全部**插件派发单，**包括它这一刻带的 `customerId` 压根没解析到任何
# 账号的情形**（待认领 / 已驳回 / 从没见过——那几种在拉取侧一律空数组，但写回侧照样通，
# 因为写回载荷不带 `customerId`，`load_team_plugin_po` 也就没有身份可比）。这是图纸
# `07:338-340` 明确划在威胁模型之外的一面，理由与「若要改须先改图纸」的完整论证见**本文件
# 头注「写回四端点的越权面」一节**。**不要在这里顺手加账号谓词**：那既与图纸相悖，也会
# 把「换号后旧单报不回来」变成资金黑洞。
#
