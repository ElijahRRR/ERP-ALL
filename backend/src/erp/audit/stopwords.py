"""R4/R5 通用词过滤 stopwords（源仓 pipelines/_stopwords.py 全量保真移植）。

R2-02 round-5 补齐：此前 pipeline 里只有最小子集(~30 词)——候选词过滤不足导致
R5 喂给 L3 的商标信号偏多（pass->reject 主链成因之一）。本模块与源仓逐字一致。
...
_NUMERIC_MIXED_RE = re.compile(r"^[\d\W]+$")  # 只含数字/非字母 (如 "12", "50 years"(空格), "4 x 4")


def is_stopword(token: str, *, min_len: int = 4) -> bool:
    """判断 token 是否应被视为 stopword (不进 R4/R5 命中)。

    token 可能是单词 ("top") 或多词短语 ("cell phone"), 小写后判断:
      - 长度 (不含空格/标点) < min_len → stopword
      - 纯数字 → stopword
      - 含字母但属于 STOPWORDS_COMMON → stopword

    多词短语 (e.g. "ninja foodi", "strawberry shortcake") 只要其中**主要的词** 在
    STOPWORDS_COMMON 里就过滤; 含字母 ≥ min_len 的多词短语默认保留 (通常是真品牌)。
    """
    if not token:
        return True
    t = token.strip().lower()
