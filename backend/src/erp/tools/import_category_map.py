"""类目映射导入 CLI（DG1：Amazon 路径 → Walmart PT，L1 类目判定弹药）。

用法：
  docker compose -f infra/docker-compose.yml exec api \\
    python -m erp.tools.import_category_map --file /data/category_map.csv

文件格式（按扩展名识别 csv/xlsx/jsonl）；列名任一即可：
  amazon_category       ← amazon_category | amazon_path | amazon_category_path | category_path
  walmart_product_type  ← walmart_product_type | walmart_pt | product_type | wpt
  confidence            ← confidence（高/中/低，自由文本）
  requires_certificate  ← requires_certificate | requires_cert（TRUE/1/是 → true）
  zh_seller_forbidden   ← zh_seller_forbidden | seller_forbidden
  requirements / notes  ← 自由文本

全局参考数据（refdata，无团队属性），走超管 system_tx。幂等：以
(amazon_category, walmart_product_type) 为键 ON CONFLICT 恒更新，重导不产生新行。
'无对应Walmart PT' 为合法业务条目（unmapped 标记），照导。
"""
...
async def _run(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise
    print(
        f"导入完成 job {job_id} [category_map]："
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="ERP 类目映射导入（refdata.category_map，全局参考数据）"
    )
